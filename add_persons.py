...
def get_feature(face_image):
    ...
    with torch.amp.autocast('cuda', enabled=recognizer.fp16):
    ...
    for name_person in os.listdir(add_persons_dir):
        ...
        for image_name in os.listdir(person_image_path):
            if image_name.endswith(("png", "jpg", "jpeg")):
                ...
                # Detect faces and landmarks using the face detector (no threshold argument)
                ...
                for i in range(len(bboxes)):
                    ...
                    if x1 < 0 or y1 < 0 or x2 > input_image.shape[1] or y2 > input_image.shape[0]:
                        print(f"Invalid bounding box coordinates for {image_name}. Skipping face.")
                        continue

                    # Extract the face from the image
                    face_image = input_image[y1:y2, x1:x2]

                    # Check if the face_image is empty
                    if face_image is None or face_image.size == 0:
                        print(f"Empty face image extracted for {image_name}. Skipping face.")
                        continue
