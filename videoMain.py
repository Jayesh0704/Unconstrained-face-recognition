...
def tracking(detector, args, rtsp_url):
    ...
    while True:
        ret, img = cap.read()
        if not ret:
            print("Failed to read from the camera stream.")
            break
...
def main():
    ...
    rtsp_url = "rtsp://169.254.54.102/media/video1"

    thread_track = threading.Thread(target=tracking, args=(detector, config_tracking, rtsp_url))
    thread_track.start()
